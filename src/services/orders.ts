...
export async function createOrder(payload: CreateOrderPayload, initialStatus: OrderStatus = 'accepted') {
...
/** Create order in one go (used when customer clicks "I've Paid") */
export async function createOrderWithPayment(payload: CreateOrderPayload) {
  // Create order with accepted status directly (no payment verification needed)
  const order = await createOrder(payload, 'accepted');

  // Optionally record payment info
...
export async function submitPayment(orderId: string, amount: number, upiRef?: string) {
  ...
  await supabase.from('notifications').insert({
    ...
    message: `Your order ${order.order_number} has been accepted. Kitchen is preparing your food.`,
    type: 'accepted',
    order_id: orderId,
  });
...
export async function getTodayStats() {
  ...
  return {
    ...
    pending: orders.filter(o => ['accepted', 'preparing', 'ready', 'out_for_delivery'].includes(o.status)).length,
